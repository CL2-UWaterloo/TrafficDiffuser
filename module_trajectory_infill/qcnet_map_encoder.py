    ...
    def forward(self, data: HeteroData) -> Dict[str, torch.Tensor]:
        ...
        if self.dataset == 'argoverse_v2':
            if self.input_dim == 2:
                ...
                # scaled_positions_pt = 2* (pos_pt - pl_map_min[pt_batch, :2]) / (pl_map_max[pt_batch, :2] - pl_map_min[pt_batch, :2]) - 1

                # x_pt = torch.concat([scaled_positions_pt, orient_pt.unsqueeze(-1)], dim=-1)
          
                # scaled_positions_pl = 2* (pos_pl - pl_map_min[pl_batch, :2]) / (pl_map_max[pl_batch, :2] - pl_map_min[pl_batch, :2]) - 1
                # x_pl = torch.concat([scaled_positions_pl, orient_pl.unsqueeze(-1)], dim=-1)
                # x_pt = data['map_point']['magnitude']
                x_pl = None
        ...
        if self.init_timestep > 1:
            x_pl = x_pl.repeat_interleave(repeats=self.init_timestep,
                                        dim=0).reshape(-1, self.init_timestep, self.hidden_dim)
