...
class ArgoverseV2Dataset(Dataset):
    ...
    Args:
        ...
        init_timestep (int, Optional): the number of historical time steps. (default: 50)
        num_infill_steps (int, Optional): the number of future time steps. (default: 60)
    ...
    def __init__(self,
                 ...
                 dim: int = 3,
                 init_timestep: int = 50,
                 num_infill_steps: int = 60,
        ...
        self.num_steps = init_timestep + num_infill_steps
        ...
        #self._num_samples = len(self._processed_file_names) // 5
        #self._processed_file_names = self._processed_file_names[:self._num_samples]
        #self._raw_file_names = self._raw_file_names[:self._num_samples]
        #########
    ...
    def get_agent_features(self, df: pd.DataFrame) -> Dict[str, Any]:
        if not self.predict_unseen_agents:  # filter out agents that are unseen during the historical time steps
            historical_df = df[df['timestep'] < self.init_timestep]
        ...
        # in_map = torch.zeros(num_agents, dtype=torch.bool)
        for track_id, track_df in df.groupby('track_id'):
            agent_idx = agent_ids.index(track_id)
            agent_steps = track_df['timestep'].values

            valid_mask[agent_idx, agent_steps] = True
            current_valid_mask[agent_idx] = valid_mask[agent_idx, self.init_timestep - 1]
            predict_mask[agent_idx, agent_steps] = True
            if self.vector_repr:  # a time step t is valid only when both t and t-1 are valid
                valid_mask[agent_idx, 1: self.init_timestep] = (
                        valid_mask[agent_idx, :self.init_timestep - 1] &
                        valid_mask[agent_idx, 1: self.init_timestep])
                valid_mask[agent_idx, 0] = False
            predict_mask[agent_idx, :self.init_timestep] = False
        ...
        if self.split == 'test':
            predict_mask[current_valid_mask
                         | (agent_category == 2)
                         | (agent_category == 3), self.init_timestep:] = True
