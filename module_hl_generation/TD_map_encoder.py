...
from utils import angle_between_2d_vectors, merge_edges, wrap_angle, weight_init

from layers import MultiHeadAttention


class TDMapEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        super(TDMapEncoder, self).__init__()
        ...
        self.m_mean = state_stats['mean'][:2].unsqueeze(0)
        self.m_std = state_stats['std'][:2].unsqueeze(0)

        self.num_latents = 16
        self.latents = nn.Parameter(torch.randn(1, self.num_latents, hidden_dim))
        self.map_summary = MultiHeadAttention(hidden_dim, num_heads)
    ...
    def forward(self, data: HeteroData) -> Dict[str, torch.Tensor]:
        ...
        if self.m_mean.device != pos_pt.device:
            self.m_mean = self.m_mean.to(pos_pt.device)
            self.m_std = self.m_std.to(pos_pt.device)

        if self.dataset == 'argoverse_v2':
            if self.input_dim == 2:
                                
                pl_batch = data['map_polygon']['batch']
                pl_map_min = data['map_min'].view(-1, 3)
                pl_map_max = data['map_max'].view(-1, 3)

                pt_batch = data['map_point']['batch']
                
                pos_pt = 2* (pos_pt - pl_map_min[pt_batch, :2]) / (pl_map_max[pt_batch, :2] - pl_map_min[pt_batch, :2]) - 1
                pos_pt = (pos_pt - self.m_mean) / self.m_std
                x_pt = torch.concat([pos_pt, orient_pt.unsqueeze(-1)], dim=-1)
                
                pos_pl = 2* (pos_pl - pl_map_min[pl_batch, :2]) / (pl_map_max[pl_batch, :2] - pl_map_min[pl_batch, :2]) - 1
                pos_pl = (pos_pl - self.m_mean) / self.m_std

                
                x_pl = torch.concat([pos_pl, orient_pl.unsqueeze(-1)], dim=-1)
        ...
        map_emb_batch = torch.split(x_pl_sum, poly_cnt_per_batch.tolist())

        map_emb = pad_sequence(map_emb_batch, batch_first=True, padding_value=0) 

        B, N_map, _ = map_emb.shape
        attn_mask_map_summary = torch.ones(B, N_map+self.num_latents, device=pos_pt.device, dtype=torch.bool)
        for i, cnt in enumerate(poly_cnt_per_batch):
            attn_mask_map_summary[i, :cnt+self.num_latents] = False
        attn_mask_map_summary = attn_mask_map_summary.unsqueeze(2) | attn_mask_map_summary.unsqueeze(1)
        ####################
        queries = self.latents.expand(B, -1, -1)
        queries = torch.cat([queries, map_emb], dim=1)  # [B, num_latents + N_map, D]

        summary = self.map_summary(queries, mask=attn_mask_map_summary)
        summary = self.norm(queries[:, :self.num_latents, :] + summary[:, :self.num_latents, :])
        
        summary = summary + self.ffn(self.norm(summary))
        
        return {'x_pt': x_pt, 'x_pl': x_pl_sum, 'map_summary': summary}